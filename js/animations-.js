jQuery(function() {
	...
	.fromTo("#about-us .caption-subtitle span:nth-of-type(1)", 0.3,
		{x:-1000,opacity:0},
...
/* -------------------------  LA RÉVOLUTION ARABICA 4 ---------------------------------- */
	...
	.fromTo("#our-products .caption-subtitle span:nth-of-type(4)", 0.4,
		...
		{x:0,opacity:1, ease:Power1.easeOut},
		"+=0.4"
	).fromTo("#our-products p", 0.5,
		{x:0,opacity:0},
		{x:0,opacity:1, ease:Power1.easeOut},
		"+=0.4"
	).fromTo("#our-products a", 0.5,
		{x:0,opacity:0},
		{x:0,opacity:1, ease:Power1.easeOut},
		"+=0.4"
	)
	...
	var revolutionDuration = $(window).height();

	var revolution = new ScrollMagic
		.Scene({
			triggerElement: '#our-products .caption-holder',	 // starting scene, when reaching this element
			offset: 0, // start scene after scrolling for n px
			duration: revolutionDuration,			 // pin the element for n px || "n%" of scrolling
			reverse: true 						// allows the effect to trigger when scrolled in the reverse direction
		})
		.triggerHook(0.4) 	// START when on trigger [Leave = 0 , center = 0.5, Enter = 1]
		.setTween(revolutionCaption)
		.setClassToggle("#our-products .caption-holder", "active") 		// add class toggle
...
function animateNumbers() {
	...
	.fromTo("#info-section .caption-subtitle span:nth-of-type(1)", 0.4,
		{x:-1000,opacity:0},
		{x:0,opacity:1, ease:Power1.easeOut}
	)
	...
	.fromTo("#advertising .caption-subtitle span", 0.5,
		{x:-1000,opacity:0},
		{x:0,opacity:1, ease:Power1.easeOut}
	).to("#trianglemorph1",2,{morphSVG:{shape:"#trianglemorph2"}})
	...
	var sagaTrameline = new TimelineMax()
	.to("#trianglemorph1",2,{morphSVG:{shape:"#trianglemorph2"}})
	...
	var scenesaga = new ScrollMagic
		...
		.triggerHook(0.5) 					// START when on trigger [Leave = 0 , center = 0.5, Enter = 1]
		.setTween(sagaTrameline) 				// Add Tween animation
		.addTo(controller);
